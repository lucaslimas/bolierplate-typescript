...
type BeforeShutdownListener = (param: any) => Promise<void>;
...
const processOnceTimeout = (signals: string[], fn: VoidFunction) => {
  return signals.forEach((sig) => process.once(sig, fn));
};
...
processOnceTimeout(SHUTDOWN_SIGNALS, forceExitAfter(SHUTDOWN_TIMEOUT));
